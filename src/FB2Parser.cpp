...
std::string
FB2Parser::fb2_author(const std::vector<XMLTag> &authors,
		      const int &variant)
{
  std::string elstr;
...
FB2Parser::fb2_description(const std::string &book)
{
  ...
  if(description.size() > 0)
    {
      std::vector<XMLTag> title_info = get_tag(description[0].content,
					       "title-info");
      if(title_info.size() > 0)
	{
	  std::vector<XMLTag> el = get_tag(title_info[0].content,
					   "author");
...
void
FB2Parser::fb2_annotation_decode(const std::string &book,
				 std::string &result,
				 const int &variant)
{
  std::vector<XMLTag> el;
	  ...
	  if(el.size() > 0)
	    {
	      ...
	      if(el.size() > 0)
		{
		  std::string ref = get_element_attribute(
		      el[0].element, "href");
		  if(!ref.empty())
		    {
		      std::string sstr = "#";
		      ...
		      if(n != std::string::npos)
			{
		      ...
		      for(auto it = el.begin(); it != el.end(); it++)
			{
			  std::string id = get_element_attribute(
			      it->element, "id");
			  if(ref == id)
			    {
			      cover = it->content;
...
void
FB2Parser::fb2_cover_fallback(const std::string &book,
			      std::string &cover)
{
  std::vector<XMLTag> el;
...
void
FB2Parser::fb2_extra_info(const std::string &book,
			  BookInfoEntry &result, const int &variant)
{
  switch(variant)
    {
    case 1:
...
void
FB2Parser::fb2_extra_info_1(const std::string &book,
	  ...
	  if(loc_el.size() > 0)
	      ...
	      for(auto it = loc_el.begin(); it != loc_el.end(); it++)
		{
		  if(result.src_language.empty())
		    ...
		    {
		      result.src_language = result.language + ", "
			  + it->content;
		    }
		}
	    }
...
void
FB2Parser::fb2_extra_info_2(const std::string &book,
			    BookInfoEntry &result)
{
  std::vector<XMLTag> el;
  el = get_tag(book, "description", false);
...
void
FB2Parser::fb2_publisher_info_1(const std::string &book,
				BookInfoEntry &result)
{
...
void
FB2Parser::fb2_publisher_info_2(const std::string &book,
				BookInfoEntry &result)
{
