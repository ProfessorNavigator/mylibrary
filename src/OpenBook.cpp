...
std::filesystem::path
OpenBook::open_book(const BookBaseEntry &bbe, const bool &copy,
		    const std::filesystem::path &copy_path,
		    const bool &find_fbd, std::function<void
		    (const std::filesystem::path &path)> open_callback)
{
  std::filesystem::path result;
  std::string ext;
  std::error_code ec;
  std::filesystem::file_status fstat = std::filesystem::symlink_status(
      bbe.file_path, ec);
  if(ec)
    {
      throw MLException(
	  std::string("\nOpenBook::open_book: ") + bbe.file_path.u8string()
	      + "\n" + ec.message() + "\n");
    }
  if(fstat.type() == std::filesystem::file_type::symlink)
    {
      std::filesystem::path resolved = std::filesystem::read_symlink(
	  bbe.file_path);
  ...
  if(ext == ".zip")
    {
      ...
      std::string conv_nm;
      bool encoding = false;
      for(int32_t i = -1; i < af->get_charset_conv_quntity(); i++)
	{
	  if(i < 0)
	    {
	  ...
	  auto it = std::find_if(files.begin(), files.end(), [search_p]
	  (auto &el)
	    {
	  ...
	  if(it != files.end())
		...
		{
		  std::filesystem::path ch_fbd = std::filesystem::u8path(
		      unpack_path);
		  ...
		  auto itfbd = std::find_if(
		      ...
		      std::bind(&OpenBook::compare_func, this,
				std::placeholders::_1, encoding, conv_nm,
				ch_fbd));
		  if(itfbd != files.end())
		    {
      ...
      if(find_fbd && loc_find_fbd)
	{
	  ...
	  auto itfbd = std::find_if(
	      files.begin(),
	      files.end(),
	      std::bind(&OpenBook::compare_func, this, std::placeholders::_1,
			false, "", ch_fbd));
	  if(itfbd != files.end())
	    {
...
void
OpenBook::correct_separators(std::vector<ZipFileEntry> &files)
{
  std::string::size_type n;
  std::string sstr = "\\";
...
bool
OpenBook::compare_func(const ZipFileEntry &ent, const bool &encoding,
