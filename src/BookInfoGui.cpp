...
BookInfoGui::creatWindow(const BookBaseEntry &bbe)
{
  ...
  clck->set_button(3);
  clck->signal_pressed().connect(
      std::bind(&BookInfoGui::show_cover_popup_menu, this,
		std::placeholders::_1, std::placeholders::_2,
		std::placeholders::_3, pop_menu));
  cover->add_controller(clck);
			...
			int height)
{
  if(cover_buf)
    {
      Glib::RefPtr<Gdk::Pixbuf> l_cover = cover_buf->scale_simple(
...
void
BookInfoGui::formPaperBookInfoSection(const BookBaseEntry &bbe, Gtk::Grid *grid,
				      int &row_num)
{
  if(bie)
    {
...
void
BookInfoGui::formEectordocInfoSection(const BookBaseEntry &bbe, Gtk::Grid *grid,
				      int &row_num)
{
  if(bie)
    {
...
void
BookInfoGui::show_cover_popup_menu(int num, double x, double y,
