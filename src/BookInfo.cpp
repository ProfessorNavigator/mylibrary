...
BookInfo::get_book_info(const BookBaseEntry &bbe)
{
  ...
  if(ec)
    {
      throw MLException(
	  "BookInfo::get_annotation error: " + ec.message() + " "
  ...
  if(ext == ".zip")
	  ...
	  if(it != files.end())
	      ...
	      else
		{
		  ...
		  std::bind(&BookInfo::compare_func, this,
			    std::placeholders::_1, encoding, conv_nm, ch_fbd));
	      if(it2 != files.end())
		{
		  bber.file_path = la.unpackByPosition(bbe.file_path, tmp,
						       *it2);
		}
	      else
		{
      ...
      if(ext != ".fb2")
	{
	  ...
	  auto it2 = std::find_if(
	      files.begin(),
	      files.end(),
	      std::bind(&BookInfo::compare_func, this, std::placeholders::_1,
			false, "", ch_fbd));
	  if(it2 != files.end())
	    {
